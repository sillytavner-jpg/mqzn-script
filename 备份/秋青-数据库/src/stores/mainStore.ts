...
export const useMainStore = defineStore('main', () => {
  ...
  function addSummary(summary: GrandSummary, upToMessageId?: number, coveredMessageIds?: number[]) {
    ...
    // 记忆分层合并：旧核心保留，AI[核心]去重后追加，近期替换
    // v1：直接使用AI输出；v2+：旧核心+AI新核心（去重），AI近期替换旧近期
    for (const mem of summary.characterMemories) {
      ...
      if (prevMem) {
        const oldCores = prevMem.coreMemories;
        const newCores = (mem.coreMemories || []).filter(
          nc => !oldCores.includes(nc),  // 只过滤完全相同的重复条目
        );
        mem.coreMemories = [...oldCores, ...newCores];
